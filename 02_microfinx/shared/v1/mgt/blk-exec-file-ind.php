...
if (isset($_POST['btn_gen_tan'])) {
  ...
  $EVENT_TYPE = "EXECUTE BULK PAYMENT FILE";
  ...
  if ($update_response=="EXECUTED") {
    ...
    if ($RESP=="EXECUTED") {
      ...
      # ... DB INSERT
      $INIT_CHANNEL = "WEB";
      $MSG_TYPE = "BULK_PAYMENT_FILE_EXECUTION_TAN";
...
if (isset($_POST['btn_exec_file'])) {
  ...
  if ($TAN_MSG_CODE=="FALSE") {
    ...
    if ( ($update_response22=="EXECUTED") && ($update_response33=="EXECUTED") ) {
      ...
      $alert_type = "SUCCESS";
      $alert_msg = "MESSAGE: Bulk payment file has been approved successfully. It is now ready for execution. Refreshing in 5 seconds.";
      $_SESSION['ALERT_MSG'] = SystemAlertMessage($alert_type, $alert_msg);
      header("Refresh:5; URL=blk-apprv-file");
    }
                ...
                <table width="100%">
                  <tr>
                    ...
                    if (($VOL_DIFF==0)&&($CNT_DIFF==0)) {
                      ...
                      ?>
                      <td width="25%">
                        <button type="submit" class="btn btn-info btn-sm pull-right" disabled="">File approval not possible. Figures are not tallying</button>
                      </td>
                      <?php
                    }
                    ?>
