...
namespace Mavuno
{
	...
	public partial class LoanAcctBalance : ContentPage
	{
        ...
        public LoanAcctBalance(ArrayList datatransfered)
        {
            try
            {
                InitializeComponent();
                ...
                // ... DisplayScreenData
                lblTitleView.Text = "Loan Bal: " + LAB.account_no;

                DisplayLoanAcctBalance();
            }
            catch (Exception mm)
            {
        ...
        #region ... 06: DisplayLoanAcctBalance
        protected void DisplayLoanAcctBalance()
        {
            Device.BeginInvokeOnMainThread(async () =>
            {
                try
                {
                    ...
                    using (UserDialogs.Instance.Loading("Processing Account Balance"))
                    {
                ...
                else
                {
                    ...
                    if (RespCode.Equals("00"))
                    {
                        ...
                        if (MifosHttpRespcode.StartsWith("2"))
                        {
                            ...
                            string Currency = MifosRespDetails.currency.code;
                            ...
                            string PrincipalOutstanding = (MifosRespDetails.summary.principalOutstanding == null) ? "" : MifosRespDetails.summary.principalOutstanding.ToString("#,##0.00");
                            string InterestOutstanding = (MifosRespDetails.summary.interestOutstanding == null) ? "" : MifosRespDetails.summary.interestOutstanding.ToString("#,##0.00");
                            string PenaltyChargesOutstanding = (MifosRespDetails.summary.penaltyChargesOutstanding == null) ? "" : MifosRespDetails.summary.penaltyChargesOutstanding.ToString("#,##0.00");

                            string TotalOutstanding = (MifosRespDetails.summary.totalOutstanding == null) ? "" : MifosRespDetails.summary.totalOutstanding.ToString("#,##0.00");

                            // ... assign to the display screen
                            lblLoanAcctNum.Text = AccountNum;
                            lblAmtDisbursed.Text = PrincipalOutstanding;
                            lblOutPrinc.Text = PrincipalOutstanding;
                            lblOutInt.Text = InterestOutstanding;
                            lblOutPen.Text = PenaltyChargesOutstanding;
                            lblOutTot.Text = Currency + " " + TotalOutstanding;
                        }
                        else
                        {
                            await DisplayAlert("App Error", "An error occurred when processing loan account details", "OK");
                        }//...end..iff03
                    }
                    else
                    {
