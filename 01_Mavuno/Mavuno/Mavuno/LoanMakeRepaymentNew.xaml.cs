...
namespace Mavuno
{
    ...
    public partial class LoanMakeRepaymentNew : ContentPage
    {
        ...
        private string[] SAB_ARRAY = { };
        private SavingsAcctBasic SELECTED_SAB = new SavingsAcctBasic();
        #endregion

        #region ... 01: Class Constructor
        public LoanMakeRepaymentNew()
        {
            try
            {
                InitializeComponent();
        ...
        public LoanMakeRepaymentNew(ArrayList datatransfered)
        {
            try
            {
                InitializeComponent();
                ...
                // ... Display Data
                lblTitleView.Text = "MoMo Loan Rpymt: " + LAB.account_no;
                lblMyPhone.Text = aes.DecryptCipheredText(WALLET.CUST_PHONE);
        ...
        private void BtnSubmit_Clicked(object sender, EventArgs e)
        {
            Device.BeginInvokeOnMainThread(async () =>
            {
                try
                {
                    using (UserDialogs.Instance.Loading("Submitting MoMo Loan Repayment"))
                    {
                        await Task.Delay(300);
                        await SubmitLoanRpymtMoMoAppln();          // ... the actual task
                    }
                }
                catch (Exception ex)
                {
        ...
        #region ... 07: SubmitLoanRpymtMoMoAppln
        private async Task SubmitLoanRpymtMoMoAppln()
        {
            try
            {
        ...
        private ArrayList ValidateDataReceived()
        {
            ...
            try
            {
                bool cnt_Msisdn = (string.IsNullOrWhiteSpace(txtMsisdn.Text)) ? false : true;
                ...
                if (cnt_Msisdn && cnt_AmountRepay && cnt_Narration)
                {
                    is_valid = true;
