...
namespace Mavuno
{
	...
	public partial class GetHelpPrev : ContentPage
	{
        ...
        public GetHelpPrev(ArrayList datatransfered)
        {
            try
            {
                InitializeComponent();
        ...
        #region ... 06: DisplayGetHelpRqstData
        protected void DisplayGetHelpRqstData()
        {
            Device.BeginInvokeOnMainThread(async () =>
            {
                try
                {
                    ...
                    using (UserDialogs.Instance.Loading("Processing Reported Issues"))
                    {
                        await Task.Delay(300);
                        await FetchReportedIssues();          // ... the actual task
                    }
                }
                catch (Exception ex)
                {
        ...
        #region ... 07: FetchReportedIssues
        private async Task FetchReportedIssues()
        {
            try
            {
                ...
                string data_to_sign = WalletId + "^" + OrgCode + "^" + CustId + "^" + MemberPhone + "^" + DeviceSerial;
                ...
                else
                {
                    ...
                    if (RespCode.Equals("00"))
                    {
                        ...
                        {
                            List<GetHelpRequest> ghr_rqst_list = new List<GetHelpRequest>();

                            for (int i = 0; i < RespPayload.Count; i++)
                            {
                                dynamic req = RespPayload[i];

                                GetHelpRequest ghr = new GetHelpRequest();
                                ghr.RECORD_ID = req.RECORD_ID;
                                ghr.ISSUE_DATE = req.ISSUE_DATE;
                                ghr.INIT_CHANNEL = req.INIT_CHANNEL;
                                ghr.CUST_ID = req.CUST_ID;
                                ghr.CUST_PHONE = req.CUST_PHONE;
                                ghr.ENTERRED_PHONE = req.ENTERRED_PHONE;
                                ghr.ENTERRED_EMAIL = req.ENTERRED_EMAIL;
                                ghr.ENTERRED_SUBJECT = req.ENTERRED_SUBJECT;
                                ghr.ENTERRED_ISSUE_DESC = req.ENTERRED_ISSUE_DESC;
                                ghr.INIT_DEVICE_ID = req.INIT_DEVICE_ID;
                                ghr.HANDLER_USER_ID = req.HANDLER_USER_ID;
                                ghr.HANDLER_DATE = req.HANDLER_DATE;
                                ghr.HANDLER_RMKS = req.HANDLER_RMKS;
                                ghr.ISSUE_STATUS = req.ISSUE_STATUS;
                                ghr.XX_RECORD_ID = "ISS000" + ghr.RECORD_ID;


                                ghr_rqst_list.Add(ghr);
                            }//..end..loop

                            prevGhrRqstListView.ItemsSource = ghr_rqst_list;
                        }//..end..iff
        ...
        private void OnListViewItemSelected(object sender, SelectedItemChangedEventArgs e)
        {
            try
            {
                GetHelpRequest ghr = e.SelectedItem as GetHelpRequest;
                ...
                datatransfered.Add(ghr);

                Navigation.PushAsync(new GetHelpPrevInfo(datatransfered));
            }
            catch (Exception mm)
            {
                DisplayAlert("Error 01", mm.Message, "OK");
