...
namespace Mavuno
{
	...
	public partial class ShareAcctBalance : ContentPage
	{
        ...
        public ShareAcctBalance(ArrayList datatransfered)
        {
            try
            {
                InitializeComponent();
        ...
        protected void DisplayShareAcctDetails()
        {
            Device.BeginInvokeOnMainThread(async () =>
            {
                try
                {
                    ...
                    using (UserDialogs.Instance.Loading("Processing Share Account Balance"))
                    {
                ...
                else
                {
                    ...
                    if (RespCode.Equals("00"))
                    {
                        ...
                        if (MifosHttpRespcode.StartsWith("2"))
                        {
                            ...
                            string CurrentMarketPrice = (MifosRespDetails.currentMarketPrice == null) ? "" : MifosRespDetails.currentMarketPrice;
                            string TotalApprovedShares = (MifosRespDetails.summary.totalApprovedShares == null) ? "" : MifosRespDetails.summary.totalApprovedShares.ToString("#,##0.00");

                            double shr_price, shr_cnt;
                            double.TryParse(CurrentMarketPrice, out shr_price);
                            double.TryParse(TotalApprovedShares, out shr_cnt);
                            double share_value = (shr_price * shr_cnt);
                            string SHR_UNIT_PRICE = shr_price.ToString("#,##0.00");
                            string SHR_VAL = share_value.ToString("#,##0.00");

                            // ... assign to the display screen
                            lblShareAcctNum.Text = AccountNum + " - " + Product;
                            lblShares.Text = TotalApprovedShares;
                            lblSharesMarketPrice.Text = SHR_UNIT_PRICE;
                            lblShareValue.Text = Currency + " " + SHR_VAL;
                        }
                        else
                        {
                            await DisplayAlert("App Error", "An error occurred when processing share account balance", "OK");
                        }//...end..iff03
                    }
                    else
                    {
