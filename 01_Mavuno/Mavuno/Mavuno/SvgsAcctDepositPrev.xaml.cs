...
namespace Mavuno
{
	...
	public partial class SvgsAcctDepositPrev : ContentPage
	{
        ...
        public SvgsAcctDepositPrev(ArrayList datatransfered)
        {
            try
            {
                InitializeComponent();
        ...
        #region ... 06: DisplayStmtRqstData
        protected void DisplayDepositRqstData()
        {
            Device.BeginInvokeOnMainThread(async () =>
            {
                try
                {
                    ...
                    using (UserDialogs.Instance.Loading("Processing Deposit Requests"))
                    {
                        await Task.Delay(300);
                        await FetchDepositRequests();          // ... the actual task
                    }
                }
                catch (Exception ex)
                {
        ...
        #region ... 07: FetchDepositRequests
        private async Task FetchDepositRequests()
        {
            try
            {
                ...
                string TranRqstType = "RQST_DEPOSIT";
                ...
                else
                {
                    ...
                    if (RespCode.Equals("00"))
                    {
                        ...
                        {
                            List<DepositRqst> deposit_rqst_list = new List<DepositRqst>();

                            for (int i = 0; i < RespPayload.Count; i++)
                            {
                                dynamic req = RespPayload[i];

                                DepositRqst deposit_rqst = new DepositRqst();
                                deposit_rqst.RECORD_ID = req.RECORD_ID;
                                deposit_rqst.DEPOSIT_REF = req.DEPOSIT_REF;
                                deposit_rqst.CHANNEL = req.CHANNEL;
                                deposit_rqst.METHOD = req.METHOD;
                                deposit_rqst.CUST_ID = req.CUST_ID;
                                deposit_rqst.SVGS_ACCT_ID_TO_CREDIT = req.SVGS_ACCT_ID_TO_CREDIT;
                                deposit_rqst.MSISDN = req.MSISDN;
                                deposit_rqst.AMOUNT_BANKED = req.AMOUNT_BANKED.ToString("#,##0.00");
                                deposit_rqst.REASON = req.REASON;
                                deposit_rqst.BANK_ID = req.BANK_ID;
                                deposit_rqst.BANK_INST_ACCT_NO = req.BANK_INST_ACCT_NO;
                                deposit_rqst.BANK_INST_ACCT_NAME = req.BANK_INST_ACCT_NAME;
                                deposit_rqst.BANK_RECEIPT_REF = req.BANK_RECEIPT_REF;
                                deposit_rqst.BANK_RECEIPT_ATTCHMT = req.BANK_RECEIPT_ATTCHMT;
                                deposit_rqst.RQST_DATE = req.RQST_DATE;
                                deposit_rqst.HANDLED_BY = req.HANDLED_BY;
                                deposit_rqst.HANDLED_ON = req.HANDLED_ON;
                                deposit_rqst.HANDLER_RMKS = req.HANDLER_RMKS;
                                deposit_rqst.APPRVD_BY = req.APPRVD_BY;
                                deposit_rqst.APPRVL_DATE = req.APPRVL_DATE;
                                deposit_rqst.APPRVL_RMKS = req.APPRVL_RMKS;
                                deposit_rqst.CORE_TXN_ID = req.CORE_TXN_ID;
                                deposit_rqst.MOMO_PROC_STATUS = req.MOMO_PROC_STATUS;
                                deposit_rqst.MOMO_TRAN_REF = req.MOMO_TRAN_REF;
                                deposit_rqst.MOMO_TELCO_TRAN_REF = req.MOMO_TELCO_TRAN_REF;
                                deposit_rqst.RQST_STATUS = req.RQST_STATUS;



                                deposit_rqst_list.Add(deposit_rqst);
                            }//..end..loop

                            prevDepositRqstListView.ItemsSource = deposit_rqst_list;
                        }//..end..iff
        ...
        private void OnListViewItemSelected(object sender, SelectedItemChangedEventArgs e)
        {
            try
            {
                DepositRqst deposit_rqst = e.SelectedItem as DepositRqst;
                ...
                datatransfered.Add(deposit_rqst);

                Navigation.PushAsync(new SvgsAcctDepositPrevInfo(datatransfered));
            }
            catch (Exception mm)
            {
                DisplayAlert("Error 01", mm.Message, "OK");
